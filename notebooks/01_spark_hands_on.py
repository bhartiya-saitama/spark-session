...
# MAGIC %md
# MAGIC # Spark Hands-On — Retail E-Commerce
# MAGIC
# MAGIC Fill in every cell marked `YOU DO`. Cells marked `DEMO` just run — read
# MAGIC the output, don't type them. If you fall behind, copy the answer from
# MAGIC `01_spark_hands_on_SOLVED.py` and keep moving; later cells depend on it.
# MAGIC
...
items_schema = StructType([
    ...
    # TODO: add the remaining five fields with the right types
])
...
items_clean = (
    items_raw
    # TODO: filter out negative quantity
    # TODO: filter out null unit_price
    # TODO: withColumn("line_total", ...)
)
print("before:", items_raw.count())
print("after :", items_clean.count())
...
top_products = (
    ...
    .agg(
        ...
        # TODO: units, orders, avg_discount
    )
    # TODO: order by revenue, descending
)
...
sales = (
    ...
    # TODO: join orders_clean on order_id — select only the columns you need first
    # TODO: join customers on customer_id
    # TODO: select the nine columns listed above
)
...
# MAGIC )
# MAGIC -- TODO: wrap product_revenue in a subquery that adds
# MAGIC --       ROW_NUMBER() OVER (PARTITION BY ... ORDER BY ... DESC) AS rank_in_category,
# MAGIC --       then keep only rank_in_category <= 3
# MAGIC SELECT * FROM product_revenue ORDER BY category, revenue DESC
